...
menuList.forEach((menu) => {
  if (menu.component) {
    if (menu.component == '#') {
      ...
      menu.component = getParentLayout()
    }
    if (menu.children) {
      menu.children.forEach((cmenu) => {
        cmenu.component = modules[`../${cmenu.component}.vue`]
        if (cmenu.children) {
          cmenu.children.forEach((ccmenu) => {
            ccmenu.component = modules[`../${ccmenu.component}.vue`]
          })
        }
      })
    }
...
export const asyncRouterMap: AppRouteRecordRaw[] = menuList
